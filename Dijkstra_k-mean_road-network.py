...
try:
    ...
    n = 50  # ile punktów
    k = 5  # ile klas
    no_of_iterations = 25  # ile iteracji
    ...
    cursor.execute(
        ...
        # f'SELECT id, geom AS geom, ST_AsText(geom) AS geomText FROM public."budynki_wawa_centroidy" ORDER BY random() limit {n} '

        f'SELECT id, geom AS geom, ST_AsText(geom) AS geomText FROM public."budynki_wawa_centroidy" '
        f'WHERE id in '
    ...
    # rozpakowuje dane o budynkach i robię df
    ...
    #################snapowanie punktow budynkow do wezlow sieci drogowej
    ...
    # rozpakowuje dane po snapie i robię df
    index_wezlow_sieci_drogowej_po_snapie = [item[0] for item in tablica_na_snap_do_wezlow_sieci_drogowej]  # indeksy punktów

    coords_wezlow_sieci_drogowej_po_snapie = [(item[1]) for item in tablica_na_snap_do_wezlow_sieci_drogowej]  # współrzedne punktów

    coordsDD_wezlow_sieci_drogowej_po_snapie = [(item[1]) for item in tablica_na_snap_do_wezlow_sieci_drogowej]  # współrzedne punktów

    coordsDX1 = [[float(item[7:-1].split()[0]), float(item[7:-1].split()[1])] for item in coordsDD_wezlow_sieci_drogowej_po_snapie]

    coordsY1_wezlow_sieci_drogowej_po_snapie = [float(item[6:-1].split()[0]) for item in coordsDD_wezlow_sieci_drogowej_po_snapie]

    coordsX1_wezlow_sieci_drogowej_po_snapie = [float(item[7:-1].split()[1]) for item in coordsDD_wezlow_sieci_drogowej_po_snapie]
    ...
    np.random.seed(0)
    centroids = np.random.choice(len(index_wezlow_sieci_drogowej_po_snapie), k, replace=False)  # indeksy centroidów z danych po snapie
    ...
    # print(' \n\n WYNIK PIERWSZEJ KLASYFIKACJI \n\n ', df_pierwsza_klasyfikacja.to_string())
    ...
    for idx, one_df in enumerate(list_of_dfs):
        # print(f'\n df wynikowy {idx}\n', one_df.to_string())
        one_df.to_csv(f'results_of_classification_500/{idx}.txt')

        plt.scatter(one_df.coordsY_budynki, one_df.coordsX_budynki, c=one_df.klasyfikacja,
                    cmap='Accent')
        # plt.plot(tmp.coordsY_budynki, tmp.coordsX_budynki, marker='o', linestyle='', markersize=3, label=idx)
        # do zrobienia centroidy w każdej iteracji
        plt.savefig(fname=f'results_of_classification_500/{idx}.png', )
        plt.title(idx)
        # plt.show()
